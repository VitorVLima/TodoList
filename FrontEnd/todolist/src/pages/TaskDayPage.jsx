...
function TasksDayPage() {
  // Consumindo os estados globais do contexto centralizado
  const {
    tasks,
    fetchTasks,
    searchTasks,
  ...
  const onAdd = async (newTask) => {
    ...
    if (res.success) {
      ...
      searchTasks("", filtroAtivo, paginaAtual, true, "");
      fetchMetrics("", filtroAtivo, true, "");
  ...
  const onUpdate = async (updatedTask) => {
    ...
    if (res.success) {
      ...
      fetchMetrics("", filtroAtivo, true, "");
      dispararSucesso(
        "A tarefa da jornada de hoje foi atualizada com sucesso.",
      );
    } else {
      tratarErrosBackend(res.error);
    }
  ...
  return (
    <>
      {/* AJUSTADO: Espaçamento entre blocos reduzido de space-y-8 para space-y-5 no mobile */}
      <div className="animate-in fade-in duration-500 space-y-5 md:space-y-8">
        <Helmet>
          <title>Tarefas de Hoje | Task Manager</title>
        </Helmet>

        {/* CABEÇALHO CUSTOMIZADO COM CARD INTEGRADO */}
        {/* AJUSTADO: Padding interno p-4 no mobile e p-6 no PC para economizar área de visualização */}
        <section className="bg-gradient-to-r from-slate-800/40 via-indigo-950/10 to-slate-800/40 border border-slate-800 p-4 md:p-6 rounded-3xl shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6">
          <div className="space-y-1">
            <span className="text-[9px] md:text-[10px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-2.5 py-0.5 md:py-1 rounded-full font-bold uppercase tracking-wider">
              Focus Mode
            </span>
            <h1 className="text-xl md:text-2xl font-black text-white tracking-tight mt-1 flex items-center gap-1.5">
              Jornada de Hoje{" "}
              <span className="text-[11px] md:text-xs text-slate-500 font-normal font-mono">
                / {filtroAtivo}
              </span>
            </h1>
            <p className="text-slate-400 text-xs md:text-sm">
              Monitore e finalize seus blocos de tarefas programados para agora.
            </p>
          </div>

          <div className="flex items-center justify-between md:justify-start gap-4 bg-slate-900/50 p-3 rounded-2xl border border-slate-800/60 w-full md:w-auto">
            <div className="space-y-0.5 min-w-[80px] md:min-w-[100px]">
            ...
            <div className="flex gap-3 border-l border-slate-800 pl-4 md:pl-6">
        ...
        {/* INDICADORES REAIS DO SERVIDOR PARA O DIA DE HOJE */}
        {/* AJUSTADO: Gap compacto (gap-2) e paddings menores no mobile para os mini-cards não quebrarem o grid */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-3">
          <div className="bg-slate-800/30 p-3 md:p-4 rounded-2xl border border-slate-800 flex items-center gap-3 md:gap-4">
            <div className="p-2.5 bg-blue-500/10 text-blue-400 rounded-xl shrink-0">
              <Clock size={16} />
            </div>
            <div className="min-w-0">
              <p className="text-[9px] md:text-[10px] uppercase font-bold text-slate-500 tracking-wider truncate">
                Pendentes
              </p>
          ...
          <div className="bg-slate-800/30 p-3 md:p-4 rounded-2xl border border-slate-800 flex items-center gap-3 md:gap-4 col-span-2 md:col-span-1">
        ...
        <section className="space-y-3 md:space-y-4 pb-6">
          <h2 className="text-[11px] md:text-sm font-bold text-slate-400 uppercase tracking-widest px-2">
            {filtroAtivo === "Todas as Tarefas"
              ? "Bloco de Tarefas"
              : `Hoje filtrado por: ${filtroAtivo}`}
          </h2>

          {tasks.length > 0 ? (
            /* CORRIGIDO: Força os cards a empilharem em uma única coluna perfeita no celular (grid-cols-1) */
            /* Evita que as ações da lixeira e de edição fiquem inacessíveis ou espremidas na lateral */
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              {tasks.map((task) => (
                <div
                  key={task.id}
                  className={`group relative bg-slate-800/40 border transition-all duration-300 p-4 md:p-5 rounded-2xl flex flex-col justify-between gap-4 h-fit
                    ${
                      task.concluida
                ...
                >
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-start gap-4">
                      <h3
                        className={`text-sm md:text-base font-bold text-white tracking-tight break-words flex-1
                        ${task.concluida ? "line-through text-slate-500 font-medium" : ""}`}
                      >
                        {task.titulo}
                      </h3>
                      <span
                        className={`px-2.5 py-0.5 rounded-md text-[9px] md:text-[10px] font-bold uppercase tracking-wider shrink-0 ${getPrioridadeBadge(task.prioridade)}`}
                      >
                        {task.prioridade}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center justify-between border-t border-slate-800/60 pt-3 mt-1">
                    ...
                    {/* Botões de ação com toque confortável no celular (p-2) e visual limpo no PC */}
                    <div className="flex items-center gap-0.5 md:gap-1 opacity-90 group-hover:opacity-100 transition-opacity">
                      {!task.concluida && (
                        <button
            ...
            <div className="bg-slate-800/10 border border-dashed border-slate-800 rounded-2xl py-12 md:py-16 flex flex-col items-center justify-center text-center px-4">
              <p className="text-xs md:text-sm italic font-medium text-slate-500">
                Nenhum bloco de foco ou tarefa activa registrada para o dia de
                hoje.
              </p>
            </div>
          )}
